...
# Негативный тест: не указано название проекта
def test_create_project_negativ():
    data = {"title": "", "users": {"f2d66cdf-21ed-4a73-9046-ec1bb366756b": "admin"}}
    resp = requests.post(base_url + '/projects', json=data, headers=headers)
    assert resp.status_code == 201
    assert resp.json()["id"] is not None

# Негативный тест: неверный метод создания проекта (get вместо post)
def test_get_project():
    data = {"title": "Newproject", "users": {"f2d66cdf-21ed-4a73-9046-ec1bb366756b": "admin"}}
    resp = requests.get(base_url + '/projects', json=data, headers=headers)
    id = resp.json()["id"]
...
# Негативный тест: в запросе на изменение проекта непрописан headers)
def test_edit_project():
    data = {"title": "First_project", "users": {"f2d66cdf-21ed-4a73-9046-ec1bb366756b": "admin"}}
    resp = requests.post(base_url + '/projects', json=data, headers=headers)
    id = resp.json()["id"]
    data = {"title": "Second_project"}
    edited = requests.put(base_url + '/projects/' + str(id), json=data)
    assert resp.json()["id"] is not None
